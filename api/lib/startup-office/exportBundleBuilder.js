...
function createStartupOfficeExportBundleBuilder(deps) {
  const {
  ...
  async function startupOfficeExportBundle({ membership, team, user }) {
    const [
  ...
  function startupOfficeExportChunkRows(collection, teamID, page, descriptor) {
    const options = { cursor: page.cursor, limit: page.request_limit };
    if (["assets", "customers", "metrics", "signals"].includes(collection)) {
  ...
  return { startupOfficeExportBundle, startupOfficeExportChunkRows };
}

module.exports = { createStartupOfficeExportBundleBuilder };
