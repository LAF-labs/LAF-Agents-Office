...
// `laf-office` binary in the background (e.g. via tmux) and point the orchestrator
// at its broker via BROKER_URL + BROKER_TOKEN env. This keeps MCP server
// resolution (os.Executable) pointing at the real laf-office, so claude/codex
...
func main() {
	...
	if brokerToken == "" {
		die("BROKER_TOKEN env required (the running laf-office's broker token)")
	}
