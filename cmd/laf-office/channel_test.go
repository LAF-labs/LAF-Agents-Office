...
func TestNewBrokerRequestUsesEnvTokenAtRequestTime(t *testing.T) {
	oldEnv := os.Getenv("LAF_OFFICE_BROKER_TOKEN")
	oldPath := brokerTokenPath
	t.Cleanup(func() {
		_ = os.Setenv("LAF_OFFICE_BROKER_TOKEN", oldEnv)
...
func TestNewBrokerRequestFallsBackToTokenFile(t *testing.T) {
	...
	if err := os.Unsetenv("LAF_OFFICE_BROKER_TOKEN"); err != nil {
...
func TestChannelViewUsesOfficeHeaderAndComposer(t *testing.T) {
	...
	view := stripANSI(m.View())
	if !strings.Contains(view, "LAF-Office") || !strings.Contains(view, "Message #general") {
...
func TestChannelViewUsesOneOnOneChrome(t *testing.T) {
	...
	if strings.Contains(view, "Welcome to LAF-Office.") || strings.Contains(view, "Message #general") {
...
func TestOneOnOneStatusBarShowsRuntimeSummary(t *testing.T) {
	...
	m.members = []channelMember{{
		Slug:         "ceo",
		Name:         "CEO",
		LiveActivity: "go test ./cmd/laf-office",
	}}
	m.tasks = []channelTask{{
		ID:      "task-1",
		Channel: "general",
...
func TestProviderSelectionSavesCodexAndRequestsRestart(t *testing.T) {
	...
	if !strings.Contains(done.notice, "Claude teammate panes were stopped.") || !strings.Contains(done.notice, "Restart LAF-Office to launch the headless Codex office runtime.") {
...
func TestTypingSwitchShortcutOpensChannelPicker(t *testing.T) {
	t.Setenv("LAF_OFFICE_API_KEY", "test-key")
	m := newChannelModel(false)
	m.channels = []channelInfo{
		{Slug: "general", Name: "general", Members: []string{"ceo", "pm"}},
		{Slug: "launch", Name: "launch", Description: "Release work", Members: []string{"ceo", "fe"}},
...
func TestNewChannelModelDoesNotAutoStartInitForDeprecatedGBrain(t *testing.T) {
	...
	t.Setenv("LAF_OFFICE_ANTHROPIC_API_KEY", "")
...
func TestChannelResetDoneImmediatelyRehydratesDirectMode(t *testing.T) {
	...
	if strings.Contains(view, "Welcome to LAF-Office.") {
...
func TestMain(m *testing.M) {
	// Use a temp home dir so tests don't read the real ~/.laf-office/config.json
