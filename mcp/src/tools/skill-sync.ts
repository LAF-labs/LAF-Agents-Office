...
interface SkillSyncState {
  last_sync: string;
  skills: Record<string, { id: string; slug: string; updated_at: string; file: string }>;
}
...
function loadSyncState(): SkillSyncState {
...
function saveSyncState(state: SkillSyncState): void {
  writeFileSync(SYNC_STATE_FILE, JSON.stringify(state, null, 2));
}

export function registerSkillSyncTools(server: McpServer, client: NexApiClient) {
  server.tool(
    "sync_skills",
    "Sync all agent skills to the local .laf-office/skills/ folder as markdown files. Local agents can read these directly instead of making API calls. Run after skill compilation or periodically.",
    {
    ...
    async ({ force }) => {
      ensureDirs();
      const state = force ? { last_sync: "", skills: {} } as SkillSyncState : loadSyncState();

      const allSkills = await client.get("/v1/agent/skills?limit=200") as {
        data: Array<{
          id: string;
          slug: string;
          title: string;
  ...
  server.tool(
    "read_skill",
    "Read a skill from the local .laf-office/skills/ folder by slug or partial name. Falls back to API if not synced locally.",
    {
    ...
    async ({ slug }) => {
      ...
      // Search local files by partial match
      ...
      try {
        ...
        // Try list and partial match
        const list = await client.get("/v1/agent/skills?limit=50") as {
          data: Array<{ id: string; slug: string; title: string }>;
        };
        const apiMatch = (list.data ?? []).find(s =>
