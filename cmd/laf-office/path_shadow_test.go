...
func TestDetectPathShadowsFindsOthers(t *testing.T) {
	...
	self := filepath.Join(selfDir, "laf-office")
	other := filepath.Join(otherDir, "laf-office")
	writeExec(t, self)
	writeExec(t, other)
...
func TestDetectPathShadowsIgnoresSelfSymlink(t *testing.T) {
	...
	real := filepath.Join(realDir, "laf-office")
	writeExec(t, real)
	link := filepath.Join(linkDir, "laf-office")
...
func TestDetectPathShadowsSkipsDirsAndNonExec(t *testing.T) {
	...
	self := filepath.Join(selfDir, "laf-office")
	writeExec(t, self)
	// A directory literally named "laf-office".
	if err := os.MkdirAll(filepath.Join(dirDir, "laf-office"), 0o755); err != nil {
		t.Fatal(err)
	}
	// A non-executable regular file named "laf-office".
	nonExec := filepath.Join(nonExecDir, "laf-office")
...
func TestDetectPathShadowsEmptyPATH(t *testing.T) {
	got := detectPathShadows("/some/path/laf-office", "")
	if got != nil {
...
// the package installs both a native `laf-office` binary and a `laf-office.js` launcher
// into the same node_modules/laf-office/bin dir. npm then creates a symlink from
// PATH (e.g. /opt/homebrew/bin/laf-office) pointing at the .js launcher, which in
// turn execs the native binary.
//
// The effect: from outside, it LOOKS like two laf-office executables on disk. In
...
func TestDetectPathShadowsIgnoresNpmSiblingLauncher(t *testing.T) {
	...
	npmBin := filepath.Join(root, "lib", "node_modules", "laf-office", "bin")
	if err := os.MkdirAll(npmBin, 0o755); err != nil {
		t.Fatalf("mkdir npmBin: %v", err)
	}
	native := filepath.Join(npmBin, "laf-office")
	writeExec(t, native)
	jsLauncher := filepath.Join(npmBin, "laf-office.js")
	...
	pathEntry := filepath.Join(pathDir, "laf-office")
