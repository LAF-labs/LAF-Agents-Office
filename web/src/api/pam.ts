...
import { brokerEventSourceIsClosed, subscribeBrokerEvent } from "./events";
...
export function subscribePamEvents(
  ...
  const onStarted = (ev: MessageEvent) => dispatch("started", ev);
  const onDone = (ev: MessageEvent) => dispatch("done", ev);
  const onFailed = (ev: MessageEvent) => dispatch("failed", ev);
  const onError = () => {
    if (closed || !brokerEventSourceIsClosed()) return;
    closed = true;
  ...
  const unsubscribes = [
    subscribeBrokerEvent("pam:action_started", onStarted as EventListener),
    subscribeBrokerEvent("pam:action_done", onDone as EventListener),
    subscribeBrokerEvent("pam:action_failed", onFailed as EventListener),
    subscribeBrokerEvent("error", onError as EventListener),
  ];
