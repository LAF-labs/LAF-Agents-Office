// laf-office-oc-probe is a tiny smoke test that drives the OpenClaw bridge client
...
func main() {
	...
	identity, err := openclaw.LoadOrCreateDeviceIdentity(os.ExpandEnv("$HOME/.laf-office/openclaw/identity.json"))
	if err != nil {
		die("identity: %v", err)
	}
