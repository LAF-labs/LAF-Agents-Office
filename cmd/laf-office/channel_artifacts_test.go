...
func TestBuildArtifactLinesShowsTaskLogsWorkflowRunsAndApprovals(t *testing.T) {
	...
	m := newChannelModel(false)
	m.tasks = []channelTask{{ID: "task-1", Title: "Ship launch notes", WorktreePath: "/tmp/laf-office-task-1"}}
	...
	if !strings.Contains(plain, "Resume in /tmp/laf-office-task-1") {
...
func writeTaskLog(t *testing.T, home, taskID, record string) {
	t.Helper()
	path := filepath.Join(home, ".laf-office", "office", "tasks", taskID)
...
func writeWorkflowRun(t *testing.T, home, provider, key, record string) {
	t.Helper()
	path := filepath.Join(home, ".laf-office", "workflows", provider)
	if err := os.MkdirAll(path, 0o755); err != nil {
