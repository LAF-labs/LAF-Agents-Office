...
// LintProvider is the narrow interface the lint pass needs for LLM
...
// NewLint constructs a Lint pass. All three arguments are required.
...
func (l *Lint) judgeCluster(ctx context.Context, entitySlug, subject, predicate string, cluster []TypedFact) (bool, string, error) {
	...
	// file for agents; the Go implementation uses a simplified inline expansion).
	...
	systemPrompt := "You are the LAF-Office lint judge. Given a cluster of facts sharing (subject, predicate), determine if there is a real semantic contradiction. Read docs/specs/WIKI-SCHEMA.md §10.4 rules: temporal validity, specificity, and paraphrase are NOT contradictions. Real contradictions are currently-valid, mutually-exclusive claims."
...
func formatLintReport(report LintReport) string {
	...
	sb.WriteString("Generated by the LAF-Office archivist lint worker. Source: wiki fact logs and graph.log.\n")
