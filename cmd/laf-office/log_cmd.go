...
// No server required — reads directly from ~/.laf-office/office/tasks/.
//
// Usage:
//
//	laf-office log              — list the 20 most recent tasks
//	laf-office log <taskID>     — dump the full JSONL for a single task as pretty lines
//	laf-office log --agent eng  — list recent tasks for a specific agent
//	laf-office log --limit 50   — override the default list size
func runLogCmd(args []string) {
	...
	fs.Usage = func() {
		...
		fmt.Fprintln(os.Stderr, "  laf-office log                 List recent tasks across all agents")
		fmt.Fprintln(os.Stderr, "  laf-office log <taskID>        Dump one task's full tool-call history")
		fmt.Fprintln(os.Stderr, "  laf-office log --agent eng     Filter the list to one agent")
		fmt.Fprintln(os.Stderr, "  laf-office log --limit 50      Override default list size")
		fmt.Fprintln(os.Stderr, "")
		fmt.Fprintln(os.Stderr, "Reads from ~/.laf-office/office/tasks/{taskID}/output.log.")
	}
	_ = fs.Parse(args)
...
func printTaskList(tasks []agent.TaskLogSummary, root string) {
	...
	fmt.Println("")
	fmt.Println("Dig into one with: laf-office log <taskID>")
}
