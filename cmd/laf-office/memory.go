...
// memory.go hosts the `laf-office memory` subcommand family. Today it owns
// one verb: `migrate`, which ports legacy Nex / GBrain memory into the
// markdown wiki at ~/.laf-office/wiki/team/.
//
// Why a separate file
// ===================
//
...
func runMemory(args []string) {
	...
	default:
		fmt.Fprintf(os.Stderr, "laf-office memory: unknown verb %q — run `laf-office memory --help` for the list.\n", verb)
		os.Exit(1)
	}
}
...
func printMemoryHelp() {
	...
	fmt.Fprintln(os.Stderr, "  migrate    Port Nex or GBrain content into ~/.laf-office/wiki/team/")
	fmt.Fprintln(os.Stderr, "")
	fmt.Fprintln(os.Stderr, "Usage:")
	fmt.Fprintln(os.Stderr, "  laf-office memory migrate --from nex [--dry-run] [--limit N]")
	fmt.Fprintln(os.Stderr, "  laf-office memory migrate --from gbrain [--dry-run] [--limit N]")
}
...
func runMemoryMigrate(args []string) {
	...
	apiKeyFlag := fs.String("api-key", "", "API key for Nex authentication (overrides LAF_OFFICE_API_KEY)")
	fs.Usage = func() {
		fmt.Fprintln(os.Stderr, "laf-office memory migrate — import legacy memory into the team wiki")
		fmt.Fprintln(os.Stderr, "")
		fmt.Fprintln(os.Stderr, "Usage:")
		fmt.Fprintln(os.Stderr, "  laf-office memory migrate --from {nex,gbrain} [--dry-run] [--limit N]")
		fmt.Fprintln(os.Stderr, "")
		fs.PrintDefaults()
	}
...
func buildAdapter(source, apiKeyOverride string) (migration.Adapter, error) {
	...
	case "nex":
		...
		if key == "" {
			return nil, fmt.Errorf("nex adapter requires an API key (set LAF_OFFICE_API_KEY or pass --api-key)")
		}
