...
// `laf-office init --help` (and similar) never fire the destructive action.
...
func printSubcommandHelp(sub string) {
	...
	case "init":
		...
		fmt.Fprintln(os.Stderr, "")
		fmt.Fprintln(os.Stderr, "This writes to ~/.laf-office/config.json. Safe to re-run.")
	case "shred", "kill":
		...
		fmt.Fprintln(os.Stderr, "  laf-office shred -y        Skip the confirmation")
		fmt.Fprintln(os.Stderr, "  laf-office kill            (alias)")
	case "import":
		fmt.Fprintln(os.Stderr, "laf-office import — pull state from another tool")
		fmt.Fprintln(os.Stderr, "")
		fmt.Fprintln(os.Stderr, "Usage:")
		fmt.Fprintln(os.Stderr, "  laf-office import --from legacy           Auto-detect a running external orchestrator")
		fmt.Fprintln(os.Stderr, "  laf-office import --from <directory>      Directory with state.json")
		fmt.Fprintln(os.Stderr, "  laf-office import --from <file.json>      Direct path to an export")
	case "memory":
	...
	case "log":
		...
		fmt.Fprintln(os.Stderr, "")
		fmt.Fprintln(os.Stderr, "Lists recent tasks from ~/.laf-office/office/tasks/ so you can see what")
		fmt.Fprintln(os.Stderr, "each agent actually did — tool by tool, with timestamps.")
		fmt.Fprintln(os.Stderr, "")
		fmt.Fprintln(os.Stderr, "Usage:")
		fmt.Fprintln(os.Stderr, "  laf-office log                     List recent tasks")
		fmt.Fprintln(os.Stderr, "  laf-office log <taskID>            Show one task in detail")
		fmt.Fprintln(os.Stderr, "  laf-office log --agent eng         Filter to one agent")
	case "mcp-team":
		fmt.Fprintln(os.Stderr, "laf-office mcp-team — start the team MCP server (used by agents, not humans)")
		fmt.Fprintln(os.Stderr, "")
		fmt.Fprintln(os.Stderr, "Usage:")
...
func main() {
	...
	if *helpAll {
		fmt.Fprintf(os.Stderr, "LAF-Office v%s — all flags (including internal):\n\n", buildinfo.Current().Version)
		flag.PrintDefaults()
		os.Exit(0)
	}
	...
	// Channel view mode (launched by laf-office team in tmux)
	if *channelView {
	...
	// Warn if another laf-office binary is on PATH and may shadow this one. Interactive
...
func runTeam(args []string, packSlug string, unsafe bool, oneOnOne bool, opusCEO bool, collabMode bool) {
	...
	fmt.Println("Team launched. Welcome to LAF-Office. Attaching...")
	fmt.Println()
...
// killRunningSession stops any running tmux or web-mode LAF-Office session.
