...
// LAF-Office broker state types (output format, mirrors internal/team unexported structs)
...
func runImport(args []string) {
	...
	fs.Usage = func() {
		...
		fmt.Fprintf(os.Stderr, "Import external orchestrator state into LAF-Office. No export step required.\n\n")
	...
	destPath := lafOfficeBrokerStatePath()
	...
	fmt.Printf("Imported %d agents, %d tasks from %s. Run laf-office to launch.\n", agentCount, taskCount, source)
}
...
func importFromLegacyDB(portOverride int) (importedBrokerState, int, int, error) {
	...
	// Only reached by `laf-office import --from legacy` against a user's local
...
func importFromJSONFile(path string) (importedBrokerState, int, int, error) {
	...
	state, agentCount, taskCount := convertToLAFOffice(pc)
...
// convertToLAFOffice transforms external orchestrator JSON state into LAF-Office broker state.
func convertToLAFOffice(pc legacyState) (importedBrokerState, int, int) {
	now := time.Now().UTC().Format(time.RFC3339)
