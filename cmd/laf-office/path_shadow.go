...
// detectPathShadows returns other laf-office executables found on pathEnv besides
...
// the PATH entry symlinks at `laf-office.js` (a launcher) but the native binary
// is `laf-office` in the same node_modules/laf-office/bin dir. They are one install,
// not a shadow.
//
...
func detectPathShadows(selfExec, pathEnv string) []string {
	...
	for _, dir := range filepath.SplitList(pathEnv) {
		...
		// not a shadow. Covers the npm layout (laf-office + laf-office.js in the same
...
// warnPathShadow writes a one-time warning to w when other laf-office executables
...
func warnPathShadow(w io.Writer) {
	...
	_, _ = fmt.Fprintln(w, "laf-office: warning: other laf-office binaries are on PATH and may shadow this one:")
	for _, s := range shadows {
		_, _ = fmt.Fprintf(w, "  %s\n", s)
	}
	_, _ = fmt.Fprintf(w, "  running: %s\n", self)
	_, _ = fmt.Fprintln(w, "  If `which laf-office` picks a different path, upgrades to this binary will have no effect until the other copy is removed.")
}
