...
func TestWakeLeadAfterSpecialistSkipsDirectMessageReply(t *testing.T) {
	l, processed, cleanup := fullDispatchLauncher(t)
	defer cleanup()

	l.broker.mu.Lock()
	l.broker.messages = append(l.broker.messages, channelMessage{
		ID:        "dm-reply",
		From:      "fe",
		Channel:   DMSlugFor("fe"),
		Content:   "DM-only answer",
		Timestamp: time.Now().UTC().Format(time.RFC3339),
	})
	l.broker.mu.Unlock()

	l.wakeLeadAfterSpecialist("fe")

	select {
	case got := <-processed:
		t.Fatalf("DM specialist reply leaked to lead wake path: %s", got)
