...
func (m channelModel) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case telegramConnectDoneMsg:
		...
		m.notice = fmt.Sprintf("Connected \"%s\" as #%s. Restart LAF-Office to activate channel sync.", msg.groupTitle, msg.channelSlug)
	...
	case tui.InitFlowMsg:
		...
		case tui.InitProviderChoice:
			m.picker = tui.NewPicker("Choose default runtime provider", tui.ProviderOptions())
			m.picker.SetActive(true)
...
func (m channelModel) runCommand(trimmed, threadTarget string) (tea.Model, tea.Cmd) {
	...
	case trimmed == "/provider":
		...
		m.notice = "Choose a default runtime provider."
		return m, nil
	case trimmed == "/cancel":
		clearCurrent()
		if m.replyToID != "" {
...
func (m channelModel) buildOneOnOneModePickerOptions() []tui.PickerOption {
	enableDescription := "Restart LAF-Office in direct mode with one selected agent and kill the rest of the Claude sessions"
	if m.isOneOnOne() {
		enableDescription = "Pick a different single agent for this direct session"
	}
	disableDescription := "Restart LAF-Office with the full office team"
	if !m.isOneOnOne() {
...
func reportChannelCrash(details string) {
	...
	fmt.Fprintln(os.Stderr, "LAF-Office channel crashed.")
