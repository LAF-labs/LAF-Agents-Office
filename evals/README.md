...
prompt drift, catches regressions before Slice 1 ships, and feeds the Week 0
benchmark corpus.
...
`must_include` and `must_not_include` are string matchers on the serialized LLM
output. `structured` is a partial JSON shape the output must match, such as
entity count, fact count, and specific predicates.
...
skip citations, the whole wiki intelligence story collapses. A
3-case-per-prompt harness catches regressions with a single LLM shell-out per
case and runs in under a minute locally.

## Adding new cases

Append to the relevant subdirectory. Keep cases small: one behavior each. When
a production regression hits, add the minimal repro case here so it does not
regress again.

## Relationship to Week 0 benchmark corpus

The Week 0 corpus (`bench/slice-1/corpus.jsonl` and `queries.jsonl`) is a larger
scale recall@20 test: 500 synthetic artifacts across 25 synthetic people and 50
