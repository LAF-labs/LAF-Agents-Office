...
| SV-I138 | Release | Secrets and config rotation are now release-gated through a machine-readable inventory with owners, cadence, emergency triggers, forbidden records, and post-rotation verification commands. | `shared/startup-office-secret-rotation.json`, `startup-office:secret-rotation` |
| SV-I139 | Release | SaaS release versioning is now release-gated through a package/schema/commit release ID, production evidence fields, rollback compatibility rules, and handoff checks. | `shared/startup-office-release-versioning.json`, `startup-office:release-versioning` |
| SV-I140 | Release | Post-release monitoring and rollback criteria are now release-gated through a health contract covering the ops monitor, synthetic monitor, 60 minute post-release window, recovery commands, and rollback triggers. | `shared/startup-office-release-health.json`, `startup-office:release-health` |
| SV-I141 | Compliance | Privacy, DPA, product terms, AI use, retention, and deletion artifacts are now packaged as a launch-controlled legal artifact manifest with versioned acceptance, commercial gating, and UI evidence. | `shared/startup-office-legal-artifacts.json`, `startup-office:legal-artifacts` |
...
| SV-I151 | Performance | API cold-start and route dispatch performance are now release-gated through a microbenchmark budget covering hosted facade cold require, hosted dispatch p95, Startup Office route-match p95, and route-contract growth. | `shared/startup-office-api-performance.json`, `startup-office:api-performance` |
| SV-I152 | Performance | Growth summary now has a release-gated query budget with explicit projections for loops, recent activity, objects, notifications, and beta ops rollups; live large-workspace benchmarking remains future hardening. | `startup-office:summary-query-budget`, `shared/startup-office-summary-query-budget.json` |
| SV-I153 | Performance | Client bundle output now has a release-gated web budget: Startup Office workspace surfaces stay lazy, large markdown/wiki dependencies are not initial-preloaded, and entry/workspace/app/vendor/CSS chunks have byte caps after production build. | `startup-office:web-bundle-budget`, `shared/startup-office-web-bundle-budget.json` |
| SV-I154 | Performance | Startup Office loop retrieval now has release-gated performance budgets: context builder uses explicit projections, bounded candidate pools, top-k caps, and output text/metadata truncation before prompt assembly; live corpus-scale search benchmarking remains future hardening. | `startup-office:retrieval-performance`, `shared/startup-office-retrieval-performance.json` |
| SV-I155 | Performance | Approvals, receipts, operating object lists, and export bundles now have cursor pagination or chunked collection export. The full bundle remains capped for a one-click snapshot, while large collections can continue through `export_chunk.pagination.next_cursor`. | `startup-office:pagination`, `startup-office:export-chunks`, `shared/startup-office-export-chunks.json` |
...
| SV-I157 | Performance | React Query polling/refetch behavior now has a release-gated web query policy: global focus refetch is disabled, SSE/broker invalidation debounces live updates, message/member/request backstops are no faster than 10s, and total poller growth is capped. | `startup-office:web-query-policy`, `shared/startup-office-web-query-policy.json` |
| SV-I158 | Performance | Database index coverage is now release-gated against large-workspace query shapes for tenant list scans, cursor export chunks, support timelines, worker claims, and outbox replay; live production-volume EXPLAIN remains future hardening. | `startup-office:index-coverage`, `shared/startup-office-index-coverage.json`, `supabase/migrations/20260526090000_add_startup_office_index_coverage.sql` |
| SV-I159 | Performance | Model-call latency is now budgeted per loop with target/warning/timeout thresholds, persisted run/job/receipt telemetry, and a release-gated manifest covering all Startup Office loop definitions. | `startup-office:model-latency-budget`, `shared/startup-office-model-latency-budget.json`, `workers/startup-office/modelLatencyBudgets.js` |
| SV-I160 | Performance | Static marketing site and hosted app assets are now budgeted together with release-gated caps for marketing bytes, app build bytes, combined bytes, file count, single JS/CSS/image files, and static marketing JS/CSS. | `startup-office:site-assets-budget`, `shared/startup-office-site-asset-budget.json`, `website/`, `web/dist` |
...
| SV-I162 | Portability | Approved company memory and non-memory operating objects can migrate from export bundles or direct payloads: memory import restores approved wiki pages, and workspace import restores assets, customers, metrics, and signals with provenance, team reassignment, storage-limit, rate-limit, audit, route, and client coverage. | `startup-office:memory-import`, `startup-office:workspace-import`, `api/lib/startup-office/workspaceImportAdapters.js` |
...
| SV-I189 | Developer experience | Static analysis now syntax-checks hosted API, Startup Office API, worker, and release scripts while CI and the release gate require zero web lint errors, warnings, or infos before typecheck and build; deeper typed API generation remains future hardening. | `startup-office:static-analysis`, `startup-office:web-lint-budget`, CI |
...
- R2/R7 now adds static-analysis coverage:
  ...
  Startup Office API, worker, and script JavaScript, then verifies CI runs the
  web lint warning budget, typecheck, and build after web dependencies are
  installed.
...
- R2 now tightens the hosted API facade size budget. The current
...
- R2 now extracts hosted orchestration intent and confirmation behavior into
  `api/lib/hosted/orchestrationHandlers.js`. The facade is down to 2,324 lines,
  the release gate runs `api/lib/hosted/orchestrationHandlers.test.js`, and
  `npm run startup-office:architecture` prevents orchestration handler logic
  from drifting back into `api/[...path].js`.
- R2 now extracts hosted public URL and allowed-origin trust logic into
  `api/lib/hosted/urlTrust.js`. The facade is down to 2,127 lines, the release
  gate runs `api/lib/hosted/urlTrust.test.js`, and
  `npm run startup-office:architecture` prevents host-header, public API base,
  private-network, and CORS origin normalizers from drifting back into
  `api/[...path].js`.
- R2 now extracts hosted audit metadata redaction into
  `api/lib/hosted/redaction.js`. The facade is down to 2,104 lines, the release
  gate runs `api/lib/hosted/redaction.test.js`, and
  `npm run startup-office:architecture` prevents bearer, LAF, GitHub, OpenAI,
  token, secret, password, and API-key redaction helpers from drifting back into
  `api/[...path].js`.
- R2 now extracts hosted user presentation and avatar normalization into
  `api/lib/hosted/userPresentation.js`. The facade is down to 2,075 lines, the
  release gate runs `api/lib/hosted/userPresentation.test.js`, and
  `npm run startup-office:architecture` prevents user serializer and avatar
  catalog logic from drifting back into `api/[...path].js`.
- R2 now extracts hosted team presentation into
  `api/lib/hosted/teamPresentation.js`. The facade is down to 2,066 lines, the
  release gate runs `api/lib/hosted/teamPresentation.test.js`, and
  `npm run startup-office:architecture` prevents team serializer logic from
  drifting back into `api/[...path].js`.
- R2 now extracts hosted security and CORS header application into
  `api/lib/hosted/securityHeaders.js`. The facade is down to 2,033 lines, the
  release gate runs `api/lib/hosted/securityHeaders.test.js`, and
  `npm run startup-office:architecture` prevents baseline security header,
  CORS, and trusted browser-origin logic from drifting back into
  `api/[...path].js`.
- R2 now extracts hosted request parsing and JSON response IO into
  `api/lib/hosted/requestIO.js`. The facade is down to 1,988 lines, the release
  gate runs `api/lib/hosted/requestIO.test.js`, and
  `npm run startup-office:architecture` prevents request path, body parsing,
  JSON byte-size, and response writer helpers from drifting back into
  `api/[...path].js`.
- R2 now extracts hosted primitive value utilities into
  `api/lib/hosted/valueUtils.js`. The facade is down to 1,949 lines, the release
  gate runs `api/lib/hosted/valueUtils.test.js`, and
  `npm run startup-office:architecture` prevents text, slug, ID, clock, flag,
  UUID, numeric, array, and compact-object helpers from drifting back into
  `api/[...path].js`.
- R2 now extracts hosted Supabase REST/RPC/Auth access into
  `api/lib/hosted/supabaseAccess.js`. The facade is down to 1,828 lines, the
  release gate runs `api/lib/hosted/supabaseAccess.test.js`, and
  `npm run startup-office:architecture` prevents Supabase URL, header, REST,
  RPC, Auth, environment, and upstream error parsing helpers from drifting back
  into `api/[...path].js`.
- R2 now extracts hosted session cookie and bearer-token handling into
  `api/lib/hosted/sessionCookies.js`. The facade is down to 1,787 lines, the
  release gate runs `api/lib/hosted/sessionCookies.test.js`, and
  `npm run startup-office:architecture` prevents session cookie parsing,
  token resolution, SameSite policy, and auth cookie write/clear logic from
  drifting back into `api/[...path].js`.
- R2 now extracts hosted ingress rate-limit keying, in-memory buckets, and
  persistent claim wiring into `api/lib/hosted/ingressRateLimits.js`. The facade
  is down to 1,757 lines, the release gate runs
  `api/lib/hosted/ingressRateLimits.test.js`, and
  `npm run startup-office:architecture` prevents ingress rate-limit state and
  claim helpers from drifting back into `api/[...path].js`.
- R2 now extracts hosted authenticated user, membership, and team resolution
  into `api/lib/hosted/userContext.js`. The facade is down to 1,741 lines, the
  release gate runs `api/lib/hosted/userContext.test.js`, and
  `npm run startup-office:architecture` prevents auth user context, active
  membership, and team resolver logic from drifting back into
  `api/[...path].js`.
- R2 now extracts hosted audit write persistence into
  `api/lib/hosted/auditWriter.js`. The facade is down to 1,712 lines, the
  release gate runs `api/lib/hosted/auditWriter.test.js`, and
  `npm run startup-office:architecture` prevents audit event and team audit
  writer logic from drifting back into `api/[...path].js`.
- R2 now extracts hosted invite email provider selection and delivery into
  `api/lib/hosted/inviteEmailDelivery.js`. The facade is down to 1,698 lines,
  the release gate runs `api/lib/hosted/inviteEmailDelivery.test.js`, and
  `npm run startup-office:architecture` prevents invite email provider and
  sender logic from drifting back into `api/[...path].js`.
- R2 now extracts Startup Office lazy runtime factories into
  `api/lib/startup-office/runtimeFactories.js`. The facade is down to 1,690
  lines, the release gate runs `api/lib/startup-office/runtimeFactories.test.js`,
  and `npm run startup-office:architecture` prevents repository, services, and
  model-client singleton factory state from drifting back into
  `api/[...path].js`.
- R2 now removes the workspace config facade wrappers by destructuring
  `STARTUP_OFFICE_WORKSPACE_CONFIG_HANDLERS` directly. The facade is down to
  1,673 lines, and `npm run startup-office:architecture` prevents workspace
  settings, config snapshot, and approval-policy wrappers from drifting back
  into `api/[...path].js`.
- R2 now removes the remaining hosted auth, onboarding, conversation, member,
  and invite route facade wrappers from `api/[...path].js`. The route dispatcher
  calls the extracted handler objects directly, the facade is down to 1,601
  lines, and `npm run startup-office:architecture` prevents those thin wrapper
  functions from returning.
- R2 now extracts repository delegate helpers into
  `api/lib/startup-office/repositoryDelegates.js`. The facade is down to 1,579
  lines, the release gate runs
  `api/lib/startup-office/repositoryDelegates.test.js`, and
  `npm run startup-office:architecture` prevents repository delegate wrappers
  from drifting back into `api/[...path].js`.
- R2 now extracts Startup Office object definitions, list reads, create
  payloads, patch payloads, and metric numeric normalization into
  `api/lib/startup-office/objectStore.js`. The facade is down to 1,439 lines,
  the release gate runs `api/lib/startup-office/objectStore.test.js`, and
  `npm run startup-office:architecture` prevents object-store helpers from
  drifting back into `api/[...path].js`.
- R2 now extracts Startup Office beta operations snapshots, billing upserts,
  terms acceptance persistence, usage rollups, storage estimation, and stuck-job
  reads into `api/lib/startup-office/operationsStore.js`. The facade is down to
  1,207 lines, the release gate runs
  `api/lib/startup-office/operationsStore.test.js`, and
  `npm run startup-office:architecture` prevents operations-store helpers from
  drifting back into `api/[...path].js`.
- R2 now extracts hosted API primitives into
  `api/lib/hosted/apiPrimitives.js`. The facade is down to 1,190 lines, the
  release gate runs `api/lib/hosted/apiPrimitives.test.js`, and
  `npm run startup-office:architecture` prevents HTTP error, request id, and
  object-value primitive helpers from drifting back into `api/[...path].js`.
- R2 now removes the final profile and demo-seed route facade function
  declarations from `api/[...path].js` by using late-bound handler references for
  the remaining initialization cycle. The facade is down to 1,179 lines, and
  `npm run startup-office:architecture` prevents those wrappers from returning.
- R2 now extracts Startup Office route-handler-map composition into
  `api/lib/startup-office/routeHandlerMap.js`. The facade is down to 1,167
  lines, the release gate runs
  `api/lib/startup-office/routeHandlerMap.test.js`, and
  `npm run startup-office:architecture` prevents the route map from drifting
  back into `api/[...path].js`.
- R2 now extracts hosted API path/method dispatch into
  `api/lib/hosted/apiRouteDispatcher.js`. The facade is down to 1,001 lines,
  the release gate runs `api/lib/hosted/apiRouteDispatcher.test.js`, and
  `npm run startup-office:architecture` prevents the hosted route if-chain from
  drifting back into `api/[...path].js`.
- R2 now extracts the hosted API request entrypoint into
  `api/lib/hosted/apiEntrypoint.js`. The facade is down to 973 lines, the
  release gate runs `api/lib/hosted/apiEntrypoint.test.js`, and
  `npm run startup-office:architecture` prevents request lifecycle and error
  handling from drifting back into `api/[...path].js`.
- R2 now extracts profile/demo-seed handler wiring into
  `api/lib/startup-office/profileSeedHandlers.js`. The facade is down to 958
  lines, the release gate runs
  `api/lib/startup-office/profileSeedHandlers.test.js`, and
  `npm run startup-office:architecture` prevents profile/demo-seed handler
  factory calls from drifting back into `api/[...path].js`.
- R2 now extracts hosted route-handler wiring into
  `api/lib/hosted/handlerBundle.js`. The facade is down to 740 lines, the
...
- R2 now extracts Startup Office object/asset/customer/import handler wiring into
  `api/lib/startup-office/objectHandlerBundle.js`. The facade is down to 696
  lines, the release gate runs
  `api/lib/startup-office/objectHandlerBundle.test.js`, and
  `npm run startup-office:architecture` prevents those handler factory calls
  from drifting back into `api/[...path].js`.
