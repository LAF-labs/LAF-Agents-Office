// laf-office-oc-probe/pack is the "pack parity" smoke test. It proves that a
// LAF-Office install with TWO OpenClaw bridge bindings behaves like a real pack:
//
...
// Run with:
//
//	OPENCLAW_TOKEN=... go run ./cmd/laf-office-oc-probe/pack
//
...
func main() {
	...
	for i, m := range members {
		raw, err := conn.Call(ctx, "sessions.create", map[string]any{
			"agentId": "main",
			"label":   "laf-office-pack-" + m.slug + "-" + runID,
		})
		if err != nil {
	...
	// Seed a temporary LAF-Office HOME so broker state doesn't clash with a real
	// install. Point identity + token back at the paired daemon.
	tmpHome, err := os.MkdirTemp("", "laf-office-oc-pack-*")
	if err != nil {
		die("tmp home: %v", err)
	}
