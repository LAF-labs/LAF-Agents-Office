// laf-office-oc-probe/bridge is a higher-level smoke test that exercises the full
// team.OpenclawBridge against a real OpenClaw daemon. Unlike the protocol-only
// probe under cmd/laf-office-oc-probe, this one proves:
//
...
// Run with:
//
//	OPENCLAW_TOKEN=... go run ./cmd/laf-office-oc-probe/bridge
package main
...
func main() {
	...
	// Load identity from the real LAF-Office path so we reuse the daemon pairing
	...
	tmpHome, err := os.MkdirTemp("", "laf-office-oc-smoke-*")
	if err != nil {
		die("tmp home: %v", err)
	}
